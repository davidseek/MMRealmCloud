...
/**
 Delete a local record, if it exists.
 
  - throws: An `NSError` if saving results in an error.
 */
public func deleteLocalRecord(recordID: CKRecordID, objectClass: RealmCloudObject.Type) throws {
    
    let realm = try! Realm()
    let id = recordID.recordName
    
    // FIXME: Unsafe realm casting
    if let object = realm.objectForPrimaryKey(objectClass as! Object.Type, key: id) {
...
/**
 Set the locally modified property.
 */
public func setModified(id: String, type: Object.Type, value: Bool) throws {
    let realm = try! Realm()
    try realm.write {
        // Update using primary key
        realm.create(type,
            value: ["id": id,
                "isLocallyModified": value],
            update: true)
    }
}

/**
 Set the locally deleted property.
 */
public func setDeleted(id: String, type: Object.Type, value: Bool) throws {
    let realm = try! Realm()
    try realm.write {
        // Update using primary key
        realm.create(type,
